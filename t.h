...
Z F b(F* y,I n){F xx=0,yy,xy=0,x_=0,y_=0,x2=0;IX(i,n,xx+=1;yy=y[i];x_+=xx;y_+=yy;xy+=xx*yy;x2+=xx*xx);_((n*xy-x_*y_)/(n*x2-x_*x_))}

#define tk(a)({mu t0=tiks();{a;};mu t1=tiks();t1-t0;})
#define $t(a)({I n=4;F ts[4];IX(m,n,F t=tk(IX(_,m+1,a));ts[m]=t);(b(ts,n)/(F)freq());})
